...
export function getUserAttribute(attribute) {
    const currentUser = getCurrentUser();
    if (currentUser != null) {
        let attr;
        currentUser.getSession(function(err, session) {
            ...
            attr = sessionIdInfo['cognito:'+attribute];
        });

        if(attr) return attr;
        else if(attribute==="groups")return "user";
    }
    else return null;    
}

export function getIdentityNum() {
    
  var id_string = getUserAttribute("groups");
  if(id_string==="admin") return 2;
  else if (id_string==="user") return 1;
  else return 0;
}